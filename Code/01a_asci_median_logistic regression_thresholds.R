...
asci<-read.csv("output_data/00_asci_delta_formatted_median.csv")
head(asci)
dim(asci)
# library(CSCI)
...
## thresholds for asci - Susie paper, table 1 - hybrid MMI 0.83
...
biol.endpoints<-c("asci_h","asci_d","asci_s")#,
...
hydro.endpoints<- colnames(asci)[8:29]
...
### CSCI endpoint
all_data_hybrid <- subset(all_data,biol.endpoints=="asci_h")
head(all_data_hybrid)

### OoverE endpoint
all_data_sba <- subset(all_data,biol.endpoints=="asci_s")
head(all_data_sba)

### MMI endpoint
all_data_dia <- subset(all_data,biol.endpoints=="asci_d")
head(all_data_dia)
...
## ASCI hybrid
ggplot(all_data_hybrid, aes(x=hydro.threshold, y=PredictedProbability, color=biol.endpoints))+
  # geom_point(size=1)+
  geom_path()+#stat_summary(fun.y="mean", geom="line")+
  facet_wrap(~hydro.endpoints, scales="free_x", nrow=4)+scale_y_continuous(limits=c(0,1))+
  geom_vline(xintercept=0, linetype="dashed")+
...
## ASCI soft bodied algae
ggplot(all_data_sba, aes(x=hydro.threshold, y=PredictedProbability, color=biol.endpoints))+
  # geom_point(size=1)+
...
## ASCI diatoms
ggplot(all_data_dia, aes(x=hydro.threshold, y=PredictedProbability, color=biol.endpoints))+
  # geom_point(size=1)+
  geom_path()+#stat_summary(fun.y="mean", geom="line")+
  facet_wrap(~hydro.endpoints, scales="free_x", nrow=4)+scale_y_continuous(limits=c(0,1))+
  geom_vline(xintercept=0, linetype="dashed")+
