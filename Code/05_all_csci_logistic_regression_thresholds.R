## each summary stat instead of different bio endpoints
...
medstat <- subset(bio_h_summary3, biol.endpoints == "CSCI")
medstat$biol.endpoints <- "Median"
head(medstat)

###### mean
new_csci <- read.csv("output_data/00_csci_delta_formatted_mean.csv")
...
meanstat <- subset(bio_h_summary3, biol.endpoints == "CSCI")
meanstat$biol.endpoints <- "Mean"
head(meanstat)
...
minstat <- subset(bio_h_summary3, biol.endpoints == "CSCI")
minstat$biol.endpoints <- "Min"
head(minstat)

#### max

new_csci <- read.csv("output_data/00_csci_delta_formatted_max.csv")
...
maxstat <- subset(bio_h_summary3, biol.endpoints == "CSCI")
maxstat$biol.endpoints <- "Max"
head(maxstat)

## join together

bio_h_summary3 <- rbind(medstat, meanstat, minstat, maxstat)
head(bio_h_summary3)
tail(bio_h_summary3)

## figures with summary stats
